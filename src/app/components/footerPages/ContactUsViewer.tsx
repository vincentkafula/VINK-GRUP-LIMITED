...
const DIRECTORY = [
  { title: "Report Fraud", icon: <AlertTriangle className="w-4 h-4" />, urgent: true, items: ["Fraud Hotline — +27 (0)61 461 5035", "Card blocking via MANSHYA App", "Suspicious transactions"] },
...
const AGENT_NETWORKS = [
  { name: "MANSHYA Head Office", type: "office" as const, icon: "🏢", cover: "8 Rose Street, Cape Town CBD", services: "Account opening, FICA verification, card collection, business banking consultations", hours: "Mon–Fri 08:00–17:00" },
...
const FAQS = [
  { q: "How do I block my card?", a: "Call the Fraud Hotline immediately, or block your card instantly from the MANSHYA App under Card Settings." },
  { q: "How do I reset my Online or Mobile Banking password?", a: "Select \"Forgot Password\" on the login screen of the MANSHYA App or web portal, and follow the verification steps." },
  ...
  { q: "Is my feedback confidential?", a: "Yes — feedback is only visible to the relevant MANSHYA support team handling your enquiry." },
];
...
function LocateTab() {
  ...
  return (
    <div className="space-y-9">
      ...
      <section>
        <p className="text-gray-400 text-xs mb-4">MANSHYA is a digital-first bank — full services at our Head Office, everyday card services nationwide via our agent network.</p>
        <div className="grid sm:grid-cols-2 gap-3">
          {results.map((a, i) => (
...
function FeedbackTab() {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(false);
    if (r.success) {
      setSubmitted("MANSHYA-" + Math.random().toString(36).slice(2, 8).toUpperCase());
      ...
      <div className="bg-gray-50 border-t border-gray-100 mt-4">
        <div className="max-w-4xl mx-auto w-full px-5 py-8 grid sm:grid-cols-2 gap-3">
          ...
          <div className="rounded-xl p-5 text-white flex items-center justify-between gap-4" style={{ background: P }}>
            <div>
              <p className="font-black text-sm flex items-center gap-2 mb-1"><Smartphone className="w-4 h-4" style={{ color: GOLD }} /> Get the MANSHYA App</p>
