 ...
 *
 * 1. MANSHYA takes a flat R1.00 fee off every tap, tracked as two named
 ...
 *    owner -- MANSHYA's per-tap split calculation has nothing to do with
 *    it, and does not touch it at all. There is deliberately no
 *    driverShare in this calculation's output.
 * 3. The investor's per-tap income is 10% of MANSHYA's own R1.00 fee
 *    specifically (R0.10/tap) -- not 10% of the fare, not 10% of what's
 *    left after MANSHYA's fee. This matches
 ...
 *    to do with.
 * 5. The owner receives everything left after MANSHYA's fee and the
 ...
 *    MANSHYA's per-tap split calculates or enforces.
 *
...
const INVESTOR_SHARE_OF_VINK_FEE_PCT = 0.10; // of MANSHYA's R1.00 fee specifically -- R0.10/tap
...
export interface RevenueSplit {
  ...
  /** Everything left after MANSHYA's flat fee only -- the investor's
   *  R0.10 is carved out from within that fee, not an additional
   *  deduction, so the owner never loses more than MANSHYA's R1.00. The
  ...
  /** True when the fare was too small to cover MANSHYA's flat fee -- the
