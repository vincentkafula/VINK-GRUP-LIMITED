...
router.post("/newsletter", (req: Request, res: Response): void => {
  ...
  if (exists) {
    res.json({ success: true, data: { message: "You are already subscribed to MANSHYA updates." } });
    return;
  }
  subscribers.push({ id: uuid(), email, createdAt: new Date().toISOString() });
  res.status(201).json({ success: true, data: { message: "You have been subscribed. Welcome to MANSHYA!" } });
});
...
router.post("/apply", (req: Request, res: Response): void => {
  ...
  const refNo = `MANSHYA-${new Date().getFullYear()}-${String(applications.length + 1001).padStart(5, "0")}`;
...
router.post("/register", (req: Request, res: Response): void => {
  ...
  const accNo = `MANSHYA${String(Math.floor(Math.random() * 9000000000) + 1000000000)}`;
  res.status(201).json({ success: true, data: { id: record.id, accountNumber: accNo, message: `Welcome to MANSHYA, ${firstName}! Your account has been created. Please complete FICA verification to activate it.` } });
});
...
router.post("/credit-check", (req: Request, res: Response): void => {
  ...
  const eligible = [
    ...
    { product: "Manshya Commuter Card",       approved: true,  reason: "Meets all criteria" },
    ...
    { product: "Manshya Gold Card",           approved: score >= 750, reason: score >= 750 ? "Pre-qualified" : "Minimum score 750 required" },
  ];
...
router.get("/branches", (_req: Request, res: Response): void => {
  res.json({ success: true, data: [
    { id: "1", name: "MANSHYA Head Office", address: "8 Rose Street, Cape Town CBD, State House Building", province: "Western Cape", lat: -33.9249, lng: 18.4241, phone: "+27210070772", hours: "Mon–Fri 08:00–17:00", services: ["Account opening", "FICA verification", "Card collection", "Device enquiries", "Business consultations"] },
    { id: "2", name: "MANSHYA Cape Town Agent (Pick n Pay Gardens)", address: "Adderley St, Cape Town", province: "Western Cape", lat: -33.9214, lng: 18.4175, phone: null, hours: "Store trading hours", services: ["Card recharge", "Cash withdrawals"] },
    { id: "3", name: "MANSHYA Agent (Shoprite Bellville)", address: "Durban Rd, Bellville", province: "Western Cape", lat: -33.9000, lng: 18.6310, phone: null, hours: "Store trading hours", services: ["Card recharge", "Cash withdrawals"] },
    { id: "4", name: "MANSHYA Agent (Spar Mitchell's Plain)", address: "Lentegeur, Mitchell's Plain", province: "Western Cape", lat: -34.0479, lng: 18.6228, phone: null, hours: "Store trading hours", services: ["Card recharge", "Airtime top-up"] },
    { id: "5", name: "MANSHYA Agent (Checkers Khayelitsha)", address: "Mew Way, Khayelitsha", province: "Western Cape", lat: -34.0359, lng: 18.6822, phone: null, hours: "Store trading hours", services: ["Card recharge", "Cash withdrawals"] },
  ]});
});
